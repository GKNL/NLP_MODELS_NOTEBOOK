...
# @Author  : Peng Miao
# @File    : HMM.py
# @Intro   : 基于HMM的中文命名实体识别

import torch

class HMM(object):
    """
    NER本质上可以看成是一种序列标注问题（预测每个字的BIOES标记）。
    在使用HMM解决NER这种序列标注问题的时候，我们所能观测到的是字组成的序列（观测序列），
    观测不到的是每个字对应的标注（状态序列）。
    """
    ...
    def train(self, word_lists, tag_lists, word2id, tag2id):
        """HMM的训练，即根据训练语料对模型参数进行估计,采用【监督学习方法】。
           因为有观测序列以及其对应的状态序列，所以
        ...
        for tag_list in tag_lists:
            ...
            for i in range(seq_len - 1):
                ...
                self.A[current_tagid][next_tagid] += 1  # 从当前状态转移到下一状态
        # 问题：如果某元素没有出现过，该位置为0，这在后续的计算中是不允许的
        # 解决方法：我们将等于0的概率加上很小的数
        self.A[self.A == 0.] = 1e-10
        self.A = self.A / self.A.sum(dim=1, keepdim=True)  # 按行相加，并且保持其二维特性（每行是一个训练样本，每行使用极大似然估计来计算aij的值）
